...
export default function HomePage() {
  return (
    <main>
      <section className="home-hero">
        <div className="section-container hero-grid">
          <div className="hero-copy">
            ...
            <p className="hero-meta">🇮🇳 Made in India <b>|</b> ⏳ Fast Turnaround Times <b>|</b> 📦 High - Low MOQs</p>
          ...
          <div className="hero-visual" aria-label="Colorful flexible packaging pouches">
            <div className="hero-gallery-column hero-gallery-up">
              {[...heroPackagingImages.slice(0, 4), ...heroPackagingImages.slice(0, 4)].map((image, index) => <Image src={image} alt="Flexible packaging pouch" className="hero-gallery-image" key={`up-${image}-${index}`} width={700} height={1000} priority={index === 0} />)}
            </div>
            <div className="hero-gallery-column hero-gallery-down">
              {[...heroPackagingImages.slice(4), ...heroPackagingImages.slice(4)].map((image, index) => <Image src={image} alt="Flexible packaging pouch" className="hero-gallery-image" key={`down-${image}-${index}`} width={700} height={1000} />)}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="faq-section"><div className="faq-intro"><p className="faq-badge">◉ FAQ&apos;S</p><h2>Got a quick question?</h2><p>We&apos;re here to help you make the right decision. Explore our frequently asked questions<br />and find answers below.</p></div><div className="faq-list">{faqs.map(([question, answer], index) => <details key={question} open={index === 0}><summary>{index + 1}. {question}<span>{index === 0 ? "×" : "+"}</span></summary><p>{answer}</p></details>)}</div></section>
    </main>
  );
}
